...
setRadioButtons(modelSelected, "Model:", models_list,
                models_list, Take_ModelSelection, 0, 3)
...
resultTextbox_scrollbar.grid(column = 1, row = 17)
# resultTextbox.pack(side="right", fill="y")
resultTextbox.grid(column = 0, row = 17)
...
#     args.log_freq = int(log_freq.get())
    ...
    #             raise StopIteration
            if allStep % args.log_freq == 0:
                ...
                training_Canvas.grid(x=3, y=0, anchor="nw")
    ...
    #                 trainer.test_step((v_X, v_X_len), v_y)
                ...
                trainingAndVal_Canvas.grid(column = 3 , row = 1)
...
ShowModel.grid(column = 0, row = 8)
            
StartButton = tk.Button(window,
                        text='Strat Training',
                        width=15, height=2,
                        command=StartTraining)
StartButton.grid(column = 0, row = 9)
